...
<section class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h5 class="m-0 text-dark">Data Outlet</h5>
                </div><!-- /.col -->
                <div class="col-sm-6">
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body table table-responsive table-sm" style="font-size: 14px;">
                            <table id="tb_outlet" class="table table-bordered table-hover table-striped js-basic-example dataTable nowrap cell-border" cellspacing="0" width="100%" role="grid">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Outlet</th>
...
<div class="modal fade" id="md-form-outlet">
    <div class="modal-dialog">
        <div class="modal-content">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="ket_outlet" name="ket_outlet" class="form-control" placeholder="Keterangan" type="text">
                                    <span class="help-block text-danger"></span>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button id='btnSaveoutlet' class="btn btn-primary btn-sm float-right" onclick="simpan_outlet()">Simpan</button>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#tb_outlet').DataTable({
            ...
            ajax: {
                "url": "<?php echo site_url('master_outlet/ajax_list') ?>",
                "type": "POST",
            },
        });
    });
    ...
    function batal() {
        $('#frm-modal-outlet')[0].reset();
        $('#md-form-outlet').modal('hide');
        $('.help-block').empty();
    ...
    function tambah_form() {
        method = 'save';
        $('#md-form-outlet').modal('show');
        $('#modal-title').text('Tambah Data outlet');
        $('#btnSaveoutlet').text('Save');
        $('.is-invalid').removeClass('is-invalid');
    }

    function simpan_outlet() {
        if (method == 'save') {
            url = '<?= site_url('master_outlet/save'); ?>';
        } else {
            url = '<?= site_url('master_outlet/update'); ?>';
        }
        $.ajax({
            url: url,
            type: 'POST',
            data: new FormData($('#frm-modal-outlet')[0]),
            dataType: 'JSON',
            contentType: false,
            processData: false,
    ...
    function edit_outlet(id) {
        ...
        $.ajax({
            url: '<?= site_url('master_outlet/edit/') ?>' + id,
            type: 'GET',
            dataType: 'JSON',
            success: function(data) {
                $('#id').val(data.id_outlet);
                $('#outlet').val(data.outlet);
                $('#ket_outlet').val(data.ket_outlet);

                $('#md-form-outlet').modal('show');
                $('#modal-title').text('Edit Data outlet');
            },
    ...
    function hapus_outlet(id) {
        swal.fire({
            ...
            if (willDelete.value) {
                $.ajax({
                    url: "<?php echo site_url('master_outlet/delete') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {

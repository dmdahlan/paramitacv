...
<section class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h5 class="m-0 text-dark">Data Tujuan</h5>
                </div><!-- /.col -->
                <div class="col-sm-6">
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body table table-responsive table-sm" style="font-size: 14px;">
                            <table id="tb_tujuan" class="table table-bordered table-hover table-striped js-basic-example dataTable nowrap cell-border" cellspacing="0" width="100%" role="grid">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>tujuan</th>
...
<div class="modal fade" id="md-form-tujuan">
    <div class="modal-dialog">
        <div class="modal-content">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input id="tujuan" name="tujuan" class="form-control" placeholder="Tujuan" type="text">
            ...
            <div class="modal-footer">
                <button id='btnSaveTujuan' class="btn btn-primary btn-sm float-right" onclick="simpan_tujuan()">Simpan</button>
...
<script type="text/javascript">
    ...
    function batal() {
        $('#frm-modal-tujuan')[0].reset();
        $('#md-form-tujuan').modal('hide');
        $('.help-block').empty();
    ...
    function tambah_form() {
        method = 'save';
        $('#md-form-tujuan').modal('show');
        $('#modal-title').text('Tambah Data Tujuan');
        $('#btnSaveTujuan').text('Save');
        $('.is-invalid').removeClass('is-invalid');
    }

    function simpan_tujuan() {
        if (method == 'save') {
            url = '<?= site_url('master_tujuan/simpan_tujuan'); ?>';
        } else {
            url = '<?= site_url('master_tujuan/update_tujuan'); ?>';
        }
        $.ajax({
            url: url,
            type: 'POST',
            data: new FormData($('#frm-modal-tujuan')[0]),
            dataType: 'JSON',
            contentType: false,
            processData: false,
    ...
    function edit_tujuan(id) {
        method = 'update';
        $('#btnSaveTujuan').text('Update');
        $.ajax({
            url: '<?= site_url('master_tujuan/edit_tujuan/') ?>' + id,
            type: 'GET',
            dataType: 'JSON',
            success: function(data) {
                $('#id').val(data.idm_tujuan);
                $('#tujuan').val(data.tujuan);
                $('#keterangan').val(data.keterangan);

                $('#md-form-tujuan').modal('show');
                $('#modal-title').text('Edit Data Tujuan');
            },
    ...
    function hapus_tujuan(id) {
        swal.fire({
            ...
            if (willDelete.value) {
                $.ajax({
                    url: "<?php echo site_url('master_tujuan/delete_tujuan') ?>/" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data) {
